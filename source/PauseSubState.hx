...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
      ...
      addVirtualPadCamera(); 
		...
		*/
		...
		FlxTween.tween(timeBar, {alpha: 1}, 0.5, {ease: FlxEase.linear});
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.keys.justPressed.P #if android || _virtualpad.buttonC.justPressed #end)
			{
		...
		#else
		if (FlxG.keys.justPressed.B #if android || controls.BACK #end)
			{
				switch (FlxG.random.int(1,7))
				{
					case 1:
