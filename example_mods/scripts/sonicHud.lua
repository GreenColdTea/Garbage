...
local numbersOffset = {10,11}
local bordersCreated = false
local textInPosition = false
local directory = ''

function onCreate()
    if getPropertyFromClass('PlayState','curStage') == 'fatality' then
        fontStyle = 'sonic3'
    elseif getPropertyFromClass('PlayState','curStage') == 'horizon' or songName == 'our-horizon' or songName == 'my-horizon-obsolete' then
        fontStyle = 'chaotix'
        numbersOffset = {7,13}
    elseif getPropertyFromClass('PlayState','curStage') == 'chotix' then
        hudTimeOffset = {22,22,50,44,17,18}
        hudTimes = {'TenMinutes','Minutes','TenSeconds','Seconds','TenMilliSeconds','MilliSeconds'}
        fontStyle = 'chotix'
        numbersOffset = {7,13}
    end
    if songName == 'fatality' or songName == 'fatality-encore' or songName == 'unresponsive' then
        oldScreen = 2
        fontStyle = 'sonic3'
    elseif songName == 'B4CKSL4SH' or songName == 'genesis' then
        fontStyle = 'sonic2'
...
function onCreatePost()
    ...
    if songName == 'our-horizon' or songName == 'long-sky' or songName  == 'long-sky-old' then
...
function onUpdate()
    ...
    if oldScreen == 3 then
        if bordersCreated == false then
            ...
            addLuaSprite('blackBorderOldSonic',true)
    ...
    if hudEnabled == true then
        ...
        setProperty('sonicTenMilliSecondsText.animation.curAnim.curFrame',math.floor((getSongPosition()/100) % 10))
        setProperty('sonicMilliSecondsText.animation.curAnim.curFrame',math.floor((getSongPosition()/10) % 10))
...
function onStepHit()
  if songName == 'our-horizon' then
    if curStep == 999 then
      enableCustomHud = false
    end
